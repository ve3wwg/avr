...
_bc_do_add(bc_num n1,bc_num n2,int scale_min) {
	...
	//////////////////////////////////////////////////////////////
	// Now add the remaining fraction part and equal
	// size integer parts.
	//////////////////////////////////////////////////////////////
...
static void
_bc_rec_mul(bc_num u,int ulen,bc_num v,int vlen,bc_num *prod,int full_scale) { 
	bc_num u0, u1, v0, v1;
	int u0len, v0len;
...
bc_raisemod(bc_num base,bc_num expo,bc_num mod,bc_num *result,int scale) {
	...
	if ( base->n_scale != 0 )
		bc_condition(bc_cond_nzero_base_scale);	// non-zero scale in base
	
	// Check the exponent for scale digits.
	if ( exponent->n_scale != 0 ) {
		bc_condition(bc_cond_nzero_exp_scale);	// non-zero scale in exponent
		bc_divide(exponent,_one_,&exponent,0); // truncate
	}
	
	// Check the modulus for scale digits.
	if ( mod->n_scale != 0 )
		bc_condition(bc_cond_nzero_mod_scale);	// non-zero scale in modulus
...
//////////////////////////////////////////////////////////////////////
// An internal routine to eliminate the need for springf(,"%ld");
//////////////////////////////////////////////////////////////////////

static char *
long2str(char *buf,unsigned bufsiz,long val) {
	char *bp = buf + bufsiz;
	int f_neg = val < 0;
	int dig, count = 0;
